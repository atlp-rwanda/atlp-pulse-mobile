...
describe('<OrgLogin />', () => {
  ...
  test('renders the organization input field and accepts input', () => {
    ...
    const pulse_co = getByTestId('devpulse.org');
    expect(pulse_co).toBeTruthy();
    expect(pulse_co.props.children).toBe('.devpulse.org');
  ...
  test('calls onSubmit with the correct data when form is valid', async () => {
    ...
    fireEvent.changeText(orgInput, 'myorg.devpulse.org');
    fireEvent.press(submitButton);

    await waitFor(() => {
      expect(mockSubmit).toHaveBeenCalledWith({ organization: 'myorg.devpulse.org' });
    });
  });
});
