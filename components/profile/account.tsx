...
import { PasswordResetSchema } from '@/validations/resetPassword.schema';
...
export default function ProfileAccountTab({ passwordUpdated }: ProfileAccountTabProps) {
  ...
  const formik = useFormik<FormValues>({
    ...
    onSubmit: async (values: FormValues) => {
      ...
      try {
        ...
        if (data) {
          toast.show(data?.resetUserPassword || 'Password updated successfully', {
            type: 'success',
          });
          passwordUpdated?.();
          formik.resetForm();
  ...
  return (
    <View className={`${bgColor} mx-4 mt-8 p-6 rounded-lg`}>
