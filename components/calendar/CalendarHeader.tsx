  ...
  return (
    <View className='flex-col'>
        <View className='flex-row justify-between items-center'>
            <Text className={`text-xl font-Inter-Medium ${colorScheme === 'light' ? 'text-[#585757]' : 'text-white'}`}>{formattedDate}</Text>
            ...
            <View className='flex-row justify-center items-center text-white gap-3'>
                ...
                <AntDesign name="left" size={16} color={colorScheme === 'light' ? '#585757' : 'white'} />
                </TouchableOpacity>
                <TouchableOpacity className='' onPress={handleNextMonth}>
                <AntDesign name="right" size={16} color={colorScheme === 'light' ? '#585757' : 'white'} />
                </TouchableOpacity>
            </View>
            </View>
        </View>
        <View className='flex-row px-4 gap-9 mt-5'>
            {DaysName.map((Day,index)=>(
                <Text key={index} className={`${colorScheme === 'light' ? 'text-[#333333]' : 'text-white'}`}>{Day.name}</Text>
            ))}
        </View>
    </View>
  )
