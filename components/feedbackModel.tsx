...
const FeedbackModal: React.FC<FeedbackModalProps> = ({ isVisible, onClose, feedbacks, textColor, bgColor }) => {
  ...
  return (
    <>
      <BlurView
        intensity={0} // You can increase intensity for a more blurred background
        tint="dark"
        style={{
          position: 'absolute',
      ...
      <View style={{ position: 'absolute', justifyContent: 'center', alignItems: 'center', zIndex: 20, margin:'auto', width:'100%' }} >
        <View className="m-auto flex-col rounded-lg  p-4 shadow-lg w-5/6  h-60 bg-[#c7d2fe] relative">
          {/* Close Icon */}
          <TouchableOpacity
            onPress={onClose}
            style={{
