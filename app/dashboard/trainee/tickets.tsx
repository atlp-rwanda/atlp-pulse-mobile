...
export default function Tickets() {
  ...
  if (ticketsLoading) return <Text>Loading...</Text>;
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
              <View>
                <View className="flex-row justify-between bg-[#c7d2fe] py-4 px-2 border-b border-[#b1b1b1] mt-5 rounded-tl-lg rounded-tr-lg">
                  ...
                  <Text className="w-32 text-sm font-bold">{t('tickets.assigner')}</Text>
                  <Text className="w-32 text-sm font-bold">{t('tickets.assignee')}</Text>
                ...
                {currentTickets.length ? (
                  currentTickets.map((ticket) => (
                    <View key={ticket.id} className="flex-row justify-around border-b border-[#b1b1b1] py-2">
                      ...
                      <Text className={`w-32 ${textColor} px-1`}>{ticket.user?.email}</Text>
                      <Text className={`w-32 ${textColor} px-1`}>{ticket.assignee?.email}</Text>
                ...
                <ActionsDropdown
                  ticketId={ticket.id}
                  onView={() => openFeedbackModal(ticket)} // Assuming this opens a modal with ticket details
